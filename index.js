...
function compareArrays(arr1, arr2) {
  ...
  // Перетворюємо в JSON-рядки для порівняння.
...
function traceBackward(n) {
  ...
  // Якщо n більше 1, викликаємо функцію traceBackward знову з аргументом n - 1.
  if (n > 1) {
    traceBackward(n - 1);
  } else {
    // Коли досягнуто 1, виводимо повідомлення про завершення рекурсії.
    console.log("Рекурсія завершена.");
  }
  // Виводимо поточне число n.
...
function displayGroupedInfo(students) {
  // Перебираємо об'єкти студентів за допомогою for of.
  for (const student of students) {
    // Виводимо повідомлення про ім'я студента.
    console.group(`Студент: ${student.name}`);
    
    // Виводимо корисне повідомлення з віком та предметом студента.
    console.log(`  Вік: ${student.age}`);
    console.log(`  Предмет: ${student.subject}`);
    
    // Закриваємо групу для кожного студента.
    console.groupEnd();
  }
...
validateUserInput({ username: '', password: "pass99999" });
// Виведе в консоль:
...
function countVowelsAndConsonants(word) {
  // Рядок, де будуть всі голосні літери
  const vowels = "aeiou";
